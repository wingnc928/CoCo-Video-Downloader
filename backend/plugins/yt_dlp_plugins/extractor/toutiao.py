...
from yt_dlp.utils.traversal import find_element, traverse_obj
...
class ToutiaoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats = []
        play_info = video_data.get('videoPlayInfo') or {}
        
        # 1. 尝试从传统的 video_list 提取
        for video in traverse_obj(play_info, (
            'video_list', lambda _, v: v['main_url'],
        )):
            formats.append({
                'url': video['main_url'],
        ...
        # 2. 尝试从现代的 dynamic_video (DASH) 提取
        dynamic_video = play_info.get('dynamic_video') or {}
        
        # 提取视频流
        dynamic_video_list = dynamic_video.get('dynamic_video_list') or []
        for i, video in enumerate(dynamic_video_list):
            main_url = video.get('main_url')
            if not main_url:
                continue
            vmeta = video.get('video_meta') or {}
            formats.append({
                'url': main_url,
                'vcodec': vmeta.get('codec_type') or 'h264',
                'acodec': 'none',
                'ext': vmeta.get('vtype') or 'mp4',
                'filesize': int_or_none(vmeta.get('size')),
                'format_id': f"video-{vmeta.get('definition') or i}",
                'fps': int_or_none(vmeta.get('fps')),
                'height': int_or_none(vmeta.get('vheight')),
                'width': int_or_none(vmeta.get('vwidth')),
                'tbr': float_or_none(ameta.get('real_bitrate') if 'ameta' in locals() else vmeta.get('real_bitrate'), scale=1000),
            })
            
        # 提取音频流
        dynamic_audio_list = dynamic_video.get('dynamic_audio_list') or []
        for i, audio in enumerate(dynamic_audio_list):
            main_url = audio.get('main_url')
            if not main_url:
                continue
            ameta = audio.get('audio_meta') or {}
            formats.append({
                'url': main_url,
                'vcodec': 'none',
                'acodec': ameta.get('audio_profile') or 'aac',
                'ext': 'm4a',
                'filesize': int_or_none(ameta.get('size')),
                'format_id': f"audio-{i}",
                'asr': int_or_none(ameta.get('audio_sample_rate')),
                'tbr': float_or_none(ameta.get('real_bitrate'), scale=1000),
            })
